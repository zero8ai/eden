...
function retryTargets(item: JsonRecord) {
  const targets: Array<{ environmentId: string; releaseId: string; label?: string }> = [];
  const add = (deployment: JsonRecord, environmentId: unknown, label?: string) => {
    if (deployment.status === "failed" && environmentId && deployment.releaseId) targets.push({ environmentId: String(environmentId), releaseId: String(deployment.releaseId), label });
  };
  add(item, item.environmentId);
  for (const deployment of item.deployments ?? []) add(deployment, deployment.environmentId ?? item.env?.id, item.env?.name);
  for (const member of item.members ?? []) for (const deployment of member.deployments ?? []) add(deployment, deployment.environmentId ?? member.envId, member.name);
  return targets;
}
...
export function ResourcesScreen() {
  ...
  if (!state.data) return <LoadingState error={state.error} />;
  const act = async (row: JsonRecord) => { const intent = row.stagedDelete ? "undo-delete" : "delete-resource"; try { const result = await submitForm(state.api(`resources/${category}`), { intent, path: row.path, ...(state.agentName ? { agent: state.agentName } : {}) }); if (result.error) throw new Error(result.error); state.refresh(); } catch (e) { Alert.alert("Resource", (e as Error).message); } };
