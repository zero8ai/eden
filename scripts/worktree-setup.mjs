...
npm run dev:tunnel
\`\`\`
This publishes the server at https://${ports.tunnelHost} and changes the child server's WorkOS callback to https://${ports.tunnelHost}/callback. Ordinary \`npm run dev\` remains localhost-only. For an ephemeral fallback, use \`npm run dev:tunnel -- --quick\`.
...
- \`npm run dev\` uses \`http://localhost:${ports.dev}/callback\`. Tunnel setup automatically registers \`https://${ports.tunnelHost}/callback\` with WorkOS. WorkOS currently provides no CLI/API operation for deleting that callback during teardown; after the Cloudflare ingress is removed it remains registered but inert.
`;
}
...
function main() {
  ...
  if (tunnelMetadata) {
    try {
      ...
      registerWorkosRedirect(
        `https://${ports.tunnelHost}/callback`,
        mainEnvParsed.WORKOS_API_KEY,
      );
    } catch (err) {
