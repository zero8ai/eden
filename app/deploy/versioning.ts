...
/** The `vN` label for the Nth release of an agent, given how many already exist. */
export function versionLabel(existingCount: number): string {
  return `v${existingCount + 1}`;
}

/**
 * Is `err` a unique-violation on the (agent, version) release label? Drizzle wraps the driver
...
export function isVersionLabelCollision(err: unknown): boolean {
  for (let e: unknown = err; e instanceof Error; e = e.cause) {
    ...
    if (pg.code === "23505" && pg.constraint_name === "releases_agent_version_uq") {
      return true;
    }
  }
  return false;
